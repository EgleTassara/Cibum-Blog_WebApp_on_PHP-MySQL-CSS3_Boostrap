...
$query_RS_commenti="SELECT * FROM commenti WHERE id_ricetta=".$_GET['codice'];
$RS_commenti=mysqli_query($connessione, $query_RS_commenti);
$row_RS_commenti=mysqli_fetch_assoc($RS_commenti);
$totalRows_commenti=mysqli_num_rows($RS_commenti);
$mysqldate = date( 'j F Y H:i:s', $phpdate);
$phpdate = $row_RS_commenti['data'];

if (isset($_POST['aggiungi'])){
	$nome=$utente['nome'];
	$commento=$_POST['commento'];
	$id_ricetta=$row_RS_dettaglio['id'];
	$username=$_SESSION['username'];
	$inserisci="INSERT INTO commenti (nome, username, id_ricetta, commento) VALUES  ('$nome', '$username', 
	'$id_ricetta', '$commento')";
	mysqli_query($connessione, $inserisci);
...
<title><?php echo $row_RS_dettaglio['nome'];?></title>
<!-- InstanceEndEditable -->
...
<div class="jumbotron">
<h1><?php echo $row_RS_dettaglio['nome'];?></h1>
<p>La ricetta di Chef Mario, suguitelo passo passo per elaborare, sperimentare, creare deliziose ricette!</p>
</div>
<!-- InstanceEndEditable -->
...
<li class="active"><?php echo $row_RS_dettaglio['nome'];?></li>
</ol>
<!-- InstanceEndEditable -->
...
<div class="col-md-9">
<div class="well">
<h1><?php echo $row_RS_dettaglio['categoria'];?>: <span class="pacifico"><?php echo $row_RS_dettaglio['nome'];?></span></h1>
<hr>

<div class="row">
  <div class="col-md-5">
    <img src="images/<?php echo $row_RS_dettaglio['foto'];?>" class="img-responsive center-block" alt="Placeholder image">
  </div>
...
<textarea name="commento" class="form-control" placeholder="Scrivi il testo del commento..."></textarea>
<p class="text-danger">Per scrivere un commento devi prima effettuare il login!</p>
<input name="aggiungi" type="submit" class="pulsanti-form" value="INVIA COMMENTO">
</form>
<hr>

<h3><?php if($totalRows_commenti>0){?>I Vostri Commenti [<?php echo $totalRows_commenti;?>]<?php }else{?>Nessun commento<?php }?></h3>

<?php if($totalRows_commenti>0){do{ ?>
<p><strong class="label label-<?php if($row_RS_commenti['username']=="damianochi@gmail.com"){echo "warning";}else{ echo "info";}?>">
<?php echo $row_RS_commenti['nome']; ?></strong><p> Ha scritto: 
<?php echo $row_RS_commenti['data']  ; ?></p>

<blockquote <?php if($row_RS_commenti['username']=="tassarae@gmail.com"){echo "warning";}?>><?php echo $row_RS_commenti['commento'];?></blockquote></p>
<?php }while($row_RS_commenti=mysqli_fetch_assoc($RS_commenti));} ?>
