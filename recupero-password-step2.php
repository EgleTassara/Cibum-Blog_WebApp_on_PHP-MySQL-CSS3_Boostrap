...
$query_RS_email="SELECT * FROM utenti WHERE email='".$_GET['email']."'";
$RS_email=mysqli_query($connessione,$query_RS_email);
$row_RS_email=mysqli_fetch_assoc($RS_email);

$errore_pass=NULL;

if($row_RS_email['email']===NULL){
	$errore_pass="La sua email non è stata trovata";
}
if($row_RS_email['email']<>NULL){
	$email=$_GET['email'];
	$password=$row_RS_email['password'];
	$mail_to="$email";
	$mail_from="deborahartino@gmail.com";
	$mail_subject="Richiesta password";
	$mail_body="<p><font color=#000000> Ecco i tuoi dati</font></p>
	User= $email <br>
	Password= $password";
	$mail_in_html="MIME-Version: 1.0\r\n";
	$mail_in_html.="Content-type: text/html; harset=iso-8859-1\r\n";
	$mail_in_html.="From: <$mail_from>";
	
	if(mail($mail_to, $mail_subject, $mail_body, $mail_in_html)){
	$errore_pass="La password è stata inviata alla sua casella email $email";
	} else {
		$errore_pass="L'email non è stata inviata";
	}
}
?>
...
<title>Recupero Password Stap 2</title>
<!-- InstanceEndEditable -->
...
<p>Complimenti sei riuscito a recuperare la tua password!<p>
</div>
<!-- InstanceEndEditable -->
...
<li><a href="recupero-password-step1.php">Library</a></li>
<li class="active">Recupero Password Step 2</li>
</ol>
<!-- InstanceEndEditable -->
